...
def canny(img):
    ...
    en = time.time()
    print(f"Edge Detection time: {(en - beg).__round__(4)} sec \n---------------------------------")
...
def mask_parameter_function():
    ...
    if switch == 1:
        ...
        cv2.imwrite('Images/mask.png', mask_)
...
cv2.createTrackbar('angle', 'window', 0, 1, on_change_angle_slider)
cv2.createTrackbar('angle weight', 'window', 1, 100, on_change_angle_slider)
cv2.createTrackbar('dist weight', 'window', 1, 100, on_change_angle_slider)
